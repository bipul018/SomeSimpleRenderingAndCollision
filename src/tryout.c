...
void swap_process_pts(ProcessNode* a, ProcessNode* b) {
    ...
    if ((a->children->next == b) && (a->children->prev == b)) {
        ...
        swap_stuff(ProcessNode *, a->children->prev,
                   b->children->prev);
        swap_stuff(ProcessNode *, a->children->next,
                   b->children->next);
        ...
        for (size_t j = 0; j < count - i - 1; ++j) {
            if ((ptr[j + 1].point->point.y < ptr[j].point->point.y)
                ...
                ((ptr[j + 1].point->point.y == ptr[j].point->point.y)
                    && (ptr[j + 1].point->point.x < ptr[j].point->
                        point.x))) {
...
void triangulate_curve(StackAllocator* stk_allocr, size_t stk_offset,
                       ProcessNode* nodes, size_t total_points,
                       union Point* base_point_ptr,
                       Triangle* triangles,
    ...
    for (size_t curr_tr = 0; curr_tr < triangle_count;) {
        ...
        if ((nodes[curr_node].children->prev == nodes[curr_node].
                children->next) ||
            (((nodes[curr_node].children->prev) < (nodes + curr_node))
                ||
                (nodes[curr_node].children->next) < (nodes +
                    curr_node))) {
        ...
        ProcessNode* greater = ((nodes[curr_node].children->next >
                nodes[curr_node].children->prev)
            ? nodes[curr_node].children->next
            : nodes[curr_node].children->prev);
        ProcessNode* lowest = (ProcessNode*)(-1);
        for (ProcessNode* ptr = nodes + curr_node + 1; ptr <= greater;
             ++ptr) {
            if ((ptr != nodes[curr_node].children->next) && (ptr !=
                nodes[curr_node].children->prev)) {
                //Test if inside triangle
                POINT p1, p2, p3;
                p1.x = nodes[curr_node].point->point.x - ptr->point->
                    point.x;
                p1.y = nodes[curr_node].point->point.y - ptr->point->
                    point.y;
                p2.x = nodes[curr_node].children->prev->point->point.x
                    - ptr->point->point.x;
                p2.y = nodes[curr_node].children->prev->point->point.y
                    - ptr->point->point.y;
                p3.x = nodes[curr_node].children->next->point->point.x
                    - ptr->point->point.x;
                p3.y = nodes[curr_node].children->next->point->point.y
                    - ptr->point->point.y;
        ...
        if (lowest != -1) {
            ...
            *(lowest->children) = chld1;
            ...
            {
                struct Child* chld = nodes[curr_node].children->next->
                    children;
            ...
            {
                struct Child* chld = nodes[curr_node].children->prev->
                    children;
