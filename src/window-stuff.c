...
int create_swapchain(StackAllocator *stk_allocr, size_t stk_offset,
    ...
    *(param.p_min_img_count) = min_img_count;
    ...
    int res = create_fences(stk_allocr, stk_offset, alloc_callbacks,
    ...
    if (res == CREATE_FENCES_ALLOC_FAILED)
        return CREATE_SWAPCHAIN_FENCE_ALLOC_FAILED;
    if (res == CREATE_FENCES_FAILED)
...
void clear_swapchain(const VkAllocationCallbacks *alloc_callbacks,
    ...
    switch (err_codes) {
        ...
        case CREATE_SWAPCHAIN_FENCE_CREATE_FAILED:
        case CREATE_SWAPCHAIN_FENCE_ALLOC_FAILED: {
            int fence_err = 0;
            if (err_codes == CREATE_SWAPCHAIN_FENCE_ALLOC_FAILED)
                fence_err = CREATE_FENCES_ALLOC_FAILED;
            if (err_codes == CREATE_SWAPCHAIN_FENCE_CREATE_FAILED)
                fence_err = CREATE_FENCES_FAILED;
            clear_fences(alloc_callbacks,
                         (ClearFencesParam) {.p_fences = &(param.p_swapchain_data->img_fences),
                                             .fences_count = param.p_swapchain_data->img_count,
                                             .device = param.device,},
                         fence_err);
        }
