...
GenerateModelOutput load_tube_solid(StackAllocator* stk_allocr,
    ...
    GenerateModelOutput out;
    out.vertex_count = 2 + face_sides * (divisions + 3);
    int triangle_count = face_sides * 2 * (1 + divisions);
    ...
    //Form the first face circle

    for (int i = 0; i < face_sides; ++i) {
        *(iptr++) = 0;
        *(iptr++) = i + 1;
        *(iptr++) = ((i + 1) % face_sides) + 1;
    }

    //Form the intermediate layers
    for (int i = 1; i <= divisions; ++i) {
        for (int j = 0; j < face_sides; ++j) {
