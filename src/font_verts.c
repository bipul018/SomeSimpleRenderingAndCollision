...
// Returns the allocated points, if add_first_point is true, then
// first point P0 is also added, and also counted, else not
size_t add_quad_curve(StackAllocator* stk_allocr,
                      size_t* p_stk_offset, POINT P0, POINT Pc,
                      POINT P1, float step_size, float error,
                      CurveNode* end_ptr, union Point* base_pt,
                      CurveNode** p_curr_curve) {
    ...
    // Set normal and flag, deciding on the particular normal
    // direction {in or out} should be done later from outside {if is
    // going to be needed}
...
struct AddCurveOutput add_font_verts(StackAllocator* stk_allocr,
    ...
    for (size_t i = 0; i < num_verts; ++i) {
        ...
        int curve_end =
            (i == (num_verts - 1)) || (p_vertices[i + 1].type ==
                STBTT_vmove);
