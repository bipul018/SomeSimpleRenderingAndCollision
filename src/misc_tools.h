...
static inline void clear_descriptor_layouts(
    const VkAllocationCallbacks* alloc_callbacks,
    ...
    case CREATE_DESCRIPTOR_LAYOUTS_OK:
        ...
        vkDestroyDescriptorSetLayout(
            param.device, *param.p_matrix_set_layout,
            alloc_callbacks);
    case CREATE_DESCRIPTOR_LAYOUTS_MATRICES_LAYOUT_FAIL:
        *param.p_matrix_set_layout = VK_NULL_HANDLE;
        ...
        vkDestroyDescriptorSetLayout(
            param.device, *param.p_lights_set_layout,
            alloc_callbacks);
...
static inline int create_descriptor_pool(
    const VkAllocationCallbacks* alloc_callbacks,
    ...
    VkDescriptorPoolSize pool_sizes[] = {
        {
            ...
            .descriptorCount = param.no_light_sets + param.
            no_matrix_sets * 2,
        },
...
static inline void clear_descriptor_pool(
    const VkAllocationCallbacks* alloc_callbacks,
...
typedef struct {
    ...
    GpuAllocrAllocatedBuffer* p_matrix_buffers;
    ...
    GpuAllocrAllocatedBuffer* p_light_buffers;
    //One buffer for each set
...
static inline int allocate_and_bind_descriptors(
...
static inline int create_model(const VkAllocationCallbacks* alloc_callbacks,
                        CreateModelParam param,
                        struct Model* out_model) {
...
static inline void clear_model(const VkAllocationCallbacks* alloc_callbacks,
...
static inline void submit_model_draw(const struct Model* p_model,
...
struct Object3D {
    ...
    union {
        ...
        struct ModelTransforms tranforms;
    };

    Vec3 color;
};

static inline Mat4 process_model_transform(Vec3 translate, Vec3 rotate,
                                    Vec3 scale) {

    Mat4 translate_mat = mat4_translate_3(
        translate.x, translate.y, translate.z);
    Mat4 rotate_mat = mat4_rotation_XYZ(rotate);
    Mat4 scale_mat = mat4_scale_3(scale.x, scale.y, scale.z);
    return mat4_multiply_mat_3(&translate_mat, &rotate_mat,
                               &scale_mat);
}

static inline PushConst object_process_push_const(struct Object3D obj) {

    PushConst pushes;
    pushes.vert_consts.model_mat =
        process_model_transform(obj.translate, obj.rotate, obj.scale);
